...
int vms_version_supplement( void)
{
  ...
  /* VMS version, OpenSSL version, if enabled. */
#ifdef HAVE_SSL
# ifdef OPENSSL_VERSION
  sts = printf( "Run-time: VMS %s %s, %s\n",
   vms_arch(), vms_vers(), OpenSSL_version( OPENSSL_VERSION));
# else /* def OPENSSL_VERSION */
  sts = printf( "Run-time: VMS %s %s, %s\n",
   vms_arch(), vms_vers(), SSLeay_version( SSLEAY_VERSION));
# endif /* def OPENSSL_VERSION [else] */
#else /* def HAVE_SSL */
  sts = printf( "Run-time: VMS %s %s\n", vms_arch(), vms_vers());
#endif /* def HAVE_SSL [else] */
 ...
 * a VMS-like path name.  sys$parse() requires a VMS-like path.
 * Decide whether to process a path as UNIX-like, using decc$to_vms().
 *
 * 2020-12-14
 * Original scheme said UNIX-like if a slash is found but that misses a
 * case like "a.b@c" (derived from, say, "http://host/a.b?c"), and the
 * "@" needs a caret-escape, which decc$to_vms() would provide.
 *
 * Revised scheme says VMS-like if ":", "[", or "<" is found.  This is
 * expected (hoped?) to put all Wget-generated paths through
 * decc$to_vms().  Another possibility would be to use a more
 * VMS-friendly FN_QUERY_SEP_STR in [.src]url.c.
 */
 ...
 *       getprogname().
 *
 *    Get the image file spec.
 */

char const *getprogname (void)
{
  static char *p = "?";         /* Default result. */
...
struct
{
    jpi_item_t img_name_itm;
    int term;
} jpi_itm_lst =
     /* buf_len  itm_cod   buf   ret_len */
     { { NAM_MAXRSS, JPI$_IMAGNAME, NULL, &img_name_len },
       0 /* Terminator. */
     };

/* Get process image name. */

  img_name_p = malloc( NAM_MAXRSS+ 1);  /* Name storage. */
  if (img_name_p != NULL)   
  {
    jpi_itm_lst.img_name_itm.buf_len = NAM_MAXRSS+ 1;
    jpi_itm_lst.img_name_itm.buf = img_name_p;

    sts = sys$getjpiw( 0, 0, 0, &jpi_itm_lst, 0, 0, 0);

    if ((sts& STS$M_SEVERITY) == STS$M_SUCCESS)
    {
      /* NUL-terminate.  Get basename.  Reduce storage. */
      img_name_p[ img_name_len] = '\0';
      p = strdup( vms_basename( img_name_p));
      free( img_name_p);
    }
  }
...
/*
 * Replacement for timegm(), an inverse of gmtime().
 *
 * 2014-12-15 SMS.  New.
 */
...
time_t timegm( struct tm *tms)
{
    time_t tt;
    char *tz_val;
    char *tz_val_save;

    /* Get (and save) the current timezone env-var value (if any). */
    tz_val = getenv( "TZ");
    if (tz_val != NULL)
    {
        tz_val_save = strdup( tz_val);
        if (tz_val_save == NULL)
        {
            return (time_t)(-1);        /* malloc() failed. */
        }
    }
    ...
    /* Use mktime() (in UTC timezone) instead of timegm(). */
    tt = mktime( tms);

    /* Restore original timezone.  Free storage. */
    if (tz_val == NULL)
    {
        unsetenv( "TZ");
    }
...
const char * secure_getenv(char *var)
{
...
static $DESCRIPTOR (logical_temp_d, "SYS$SCRATCH");
static const $DESCRIPTOR (table_d, "LNM$JOB");

equiv_d.dsc$w_length = sizeof (equiv) - 1;

if ( strcmp(var,"TMPDIR") == 0 )
{
	r0_status = lib$get_logical (&logical_temp_d,
                                     &equiv_d,
	...
	if (r0_status != 1)
	{
		...
		equiv_d.dsc$w_length=1;
	}

}
return equiv_d.dsc$a_pointer;
}
